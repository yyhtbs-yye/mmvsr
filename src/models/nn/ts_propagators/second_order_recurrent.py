...
from src.models.archs import ResidualBlockNoBN
from src.models.utils import flow_warp, make_layer
...
class SecondOrderRecurrentPropagator(BaseModule):
    ...
    def forward(self, curr_feats, flows, prev_feats=[]):
        ...
        for i in range(0, t):
            ...
            n1_feat = curr_feat
            n1_flow = curr_feats.new_zeros(n, 2, h, w)
            n1_cond = curr_feat
            n2_feat = curr_feat
...
<<<<<<< HEAD:mmvsr/models/_temporal_propagator/second_order_recurrent.py
...
=======
            ...
            # Concatenate conditions for deformable convolution.
            n12c_cond = torch.cat([n2_cond, n1_cond, curr_feat], dim=1)
            # Concatenate features for deformable convolution.
            n12_feat = torch.cat([n2_feat, n1_feat], dim=1)
            # Use deformable convolution to refine the offset (coarse='n1_flow','n2_flow'),
            # then apply it to align 'prop_feat'
            align_feat = self.aligner(n12_feat, n12c_cond, [n1_flow, n2_flow])
>>>>>>> c6610254e8531bb6ef3ffe54fecd2367e9d1571f:src/models/nn/ts_propagators/second_order_recurrent.py
